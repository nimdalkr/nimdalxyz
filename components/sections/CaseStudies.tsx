...
function CaseStudyCard({
  ...
  return (
    <motion.article
      id={item.slug}
      className="case-card glass-panel project-card group overflow-hidden p-4 transition duration-300 hover:border-[var(--cyan)]/55 hover:shadow-[0_0_56px_rgba(85,214,194,0.16)] md:p-5"
      whileHover={shouldReduceMotion ? undefined : { y: -8, rotate: index % 2 ? -0.25 : 0.25 }}
      transition={{ duration: 0.36, ease: [0.16, 1, 0.3, 1] }}
    >
      <div className="flex h-full min-h-[760px] flex-col justify-between gap-8">
        <div>
          <div className="project-media relative aspect-[16/10] overflow-hidden border border-white/12 bg-[#061312]">
            <Image
              src={item.media.src}
              alt={item.media.alt}
              fill
            ...
            />
            <div className="project-media-vignette absolute inset-0" aria-hidden />
            <div className="project-media-scan absolute inset-0" aria-hidden />
            <div className="absolute left-4 top-4 flex items-center gap-2 border border-white/14 bg-black/48 px-3 py-2 text-xs font-black uppercase tracking-[0.14em] text-white/70 backdrop-blur-md">
              <ScanLine className="h-4 w-4 text-[var(--cyan)]" aria-hidden />
              {item.media.cue}
            </div>
            <div className="absolute bottom-4 right-4 font-[var(--font-mono)] text-xs font-black text-[var(--acid)]">
          ...
          <div className="mt-7 flex items-start justify-between gap-6">
            ...
            <a
              ...
              className="grid h-11 w-11 place-items-center rounded-md border border-white/12 text-white/68 transition hover:border-white/45 hover:bg-white/[0.06] hover:text-white"
            >
              <ArrowUpRight className="h-5 w-5" />
            </a>
          </div>

          <h3 className="mt-7 max-w-xl font-[var(--font-display)] text-4xl font-black uppercase leading-none md:text-5xl">
            {item.title}
          </h3>
          <p className="mt-5 text-lg leading-8 text-white/68">{item.oneLiner}</p>
          <p className="mt-7 border-l border-[var(--acid)]/50 pl-5 text-sm leading-7 text-white/50">
            {item.context}
          </p>
